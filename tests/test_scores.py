...
def test_compare_model_scores_regression():
    ...
    with patch('mds_2025_helper_functions.scores.cross_validate', return_value = mock_cv_results):
        result_r = compare_model_scores(dummy_r, tree_r, X=x, y=y_regression, return_train_scores=True)
...
def test_compare_model_scores_classification():
    ...
    with patch('mds_2025_helper_functions.scores.cross_validate', return_value = mock_cv_results):
        result_c = compare_model_scores(dummy_c, tree_c, X=x, y=y_classification, return_train_scores=True)
...
def test_compare_model_scores_same_model_type():
    ...
    with patch('mds_2025_helper_functions.scores.cross_validate', return_value = mock_cv_results):
        result = compare_model_scores(tree_r, tree_r2, X=x, y=y_regression, return_train_scores=True)
...
def test_compare_model_scores_single_model():
    ...
    with pytest.raises(TypeError) as e:
        compare_model_scores(dummy_r, X=x, y=y_regression, return_train_scores=True)
...
def test_compare_model_scores_invalid_model():
    ...
    with pytest.raises(TypeError) as e:
        compare_model_scores(dummy_r, non_sklearn_object, X=x, y=y_regression, return_train_scores=True)
...
def test_compare_model_scores_mixed_models():
    ...
    with pytest.raises(ValueError) as e:
        compare_model_scores(dummy_r, dummy_c, X=x, y=y_regression)
